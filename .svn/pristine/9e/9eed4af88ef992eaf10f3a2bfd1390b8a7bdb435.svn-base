 ...
 * @LastEditTime: 2022-07-08 15:25:54
 * @Description: TO DO
 */
'use strict';
...
var textField = function (options, defaultParameters) {
    ...
    // 任务参与人列表单独加的内容
    let isCollection = options.id == 'multiInstance-collection'
    let htmlStr = '<span id="dsj" class="select-user-btn">选择用户</span>'
    ...
    resource.html =
        `<label for="activiti-${escapeHTML(resource.id)}" ${(canBeDisabled ? 'data-disable="isDisabled" ' : '')}` +
        (canBeHidden ? 'data-show="isHidden" ' : '') +
        (dataValueLabel ? 'data-value="' + escapeHTML(dataValueLabel) + '"' : '') + '>' + escapeHTML(label) + '</label>' +
        (isCollection ? htmlStr : '') + //加入按钮
