<template>
	<div class="app-container">
		...
		<el-table v-loading="loading" border :data="Modeler">
			...
			<el-table-column label="部署时间" align="center" width="95" prop="deploymentTime" />
		...
		/>
		<!--bpmnjs在线流程设计器-->
		<el-dialog :visible.sync="modelVisible" title="流程图" width="1200px" @close="modelCancel" append-to-body>
		...
		<!-- 查看流程图 -->
		<el-dialog title="查看流程" :visible.sync="showBpmn" width="70%" append-to-body>
...
<script>
import { listDefinition, delDefinition, suspendOrActiveApply, getDefinitionXML } from '@/api/activiti/definition'
...
export default {
	...
	methods: {
		readProcess({ resourceName, deploymentId }) {
			this.showBpmn = true
			getDefinitionXML({ resourceName, deploymentId }).then((response) => {
				this.$refs.BpmnViewer.createNewDiagram(response)
			})
		},
		/** 查询Definition列表 */
		getList() {
		...
		OnlineModificationProcess(data) {
			...
			localStorage.setItem('VUE_APP_BASE_API', process.env.VUE_APP_BASE_API)
			// this.modelerUrl =
			// 	'/activiti/definition/edit?type=editBpmn&deploymentFileUUID=' +
			// 	data.deploymentId +
			// 	'&deploymentName=' +
