...
const name = defaultSettings.title || '宏鹏OA协同办公系统' // 标题
...
// 这里只列一部分，具体配置参考文档
module.exports = {
    // webpack默认不会对node_modules中的插件进行转译,手动添加对'sockjs-client' 转译
                    ...
                    config.plugin('FileManagerPlugin').use(FileManagerPlugin, [{
                        events: {
                            onEnd: {
                                ...
                                archive: [{
                                    source: './dist',
                                    destination: `./dist_zip/OA_dist_${process.env.VUE_APP_MODE} ${new Date().Format("yyyy-MM-dd hh'mm'ss")}.zip`
                                }]
                            }
                        }
                    }])
