...
const layouts = {
	...
	tableList(h, element, index, parent) {
		const { activeItem } = this.$listeners
		const className =
			this.activeId === element.__config__.formId ? 'drawing-row-item active-from-item' : 'drawing-row-item'
		let child = renderChildren.apply(this, arguments)
		...
		return (
			<el-col span={element.__config__.span}>
				...
				>
					<span class="component-name">{element.__config__.componentName}</span>
					<el-divider content-position="left">{element.titleText}</el-divider>
                    <p class="tip">请添加组件</p>
					<draggable
						list={element.__config__.children}
						animation={340}
						group="componentsGroup"
						class="drag-wrapper clearfix"
					>
						{child}
					</draggable>
					{components.itemBtns.apply(this, arguments)}
					...
					<div class="btn-row">
						<el-button>{element.buttonText}</el-button>
					</div>
				</el-row>
			</el-col>
		)
