	...
	<div class="containers vue-bpmn-diagram-container">
		...
		<ul class="buttons" v-show="showEditor">
			...
			<li class="item submit">
				<a class="btn" id="js-download-diagram" @click="saveAndDeploy()"> 保存并部署 </a>
			</li>
		</ul>
	</div>
</template>
...
<script>
import Vue from 'vue'
import Append from './userSelect.vue'
...
export default {
	...
	mounted: function () {
		...
		eventBus.on('element.click', (e) => {
			if (e.element.type == 'bpmn:UserTask') {
				this.$nextTick(() => {
					let el = document.getElementById('dsj')
					console.warn(el)
					if (el) {
						//创建一个构造器
						var Profile = Vue.extend(Append)
						new Profile().$mount('#dsj')
					}
				})
			}
		})
	...
	methods: {
		...
		saveAndDeploy() {
			...
			// var e = elementRegistry.get('Process_1')
			// console.log(e)
			// return
			...
			this.$confirm('确认保存并部署模型, 是否继续?', '提示', {
				...
				.then(() => {
					this.bpmnModeler.saveXML({ format: true }, function (err, xml) {
						...
						// 创建新的
						addDeploymentByString({ stringBPMN: xml })
							.then((result) => {
								if (result.code === 100) {
									self.$message({
										type: 'success',
